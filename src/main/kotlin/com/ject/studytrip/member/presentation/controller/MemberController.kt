...
class MemberController(
    ...
    @Operation(summary = "멤버 수정", description = "특정 멤버의 이름 또는 카테고리를 수정합니다.")
    @PatchMapping("/me")
    fun updateMember(
        @AuthenticationPrincipal memberId: String,
        @RequestBody @Valid request: UpdateMemberRequest,
    ): ResponseEntity<StandardResponse> {
        memberFacade.updateMember(memberId.toLong(), request)
    ...
    @Operation(summary = "멤버 삭제", description = "특정 멤버를 삭제합니다. (회원탈퇴)")
    @DeleteMapping("/me")
    fun deleteMember(
        @AuthenticationPrincipal memberId: String,
    ): ResponseEntity<StandardResponse> {
        memberFacade.deleteMember(memberId.toLong())
    ...
    fun restoreMember(
        @PathVariable @NotNull(message = "멤버 ID는 필수 요청 파라미터입니다.") memberId: Long,
    ): ResponseEntity<StandardResponse> {
    ...
    @Operation(summary = "멤버 즉시 삭제", description = "특정 멤버를 즉시 삭제하고, 관련된 모든 데이터를 즉시 삭제한다. (CASCADE)")
    @DeleteMapping("/me/hard-delete")
    fun hardDeleteMember(
        @AuthenticationPrincipal memberId: String,
    ): ResponseEntity<StandardResponse> {
        memberFacade.hardDeleteMemberCascade(memberId.toLong())
    ...
    @Operation(summary = "멤버 상세 조회", description = "특정 멤버를 상세 조회합니다.")
    @GetMapping("/me")
    fun loadMember(
        @AuthenticationPrincipal memberId: String,
    ): ResponseEntity<StandardResponse> {
        val result = memberFacade.getMemberDetail(memberId.toLong())
        ...
        return ResponseEntity
            ...
            .body(
                StandardResponse.success(
                    HttpStatus.OK.value(),
                    LoadMemberDetailResponse.of(result.memberInfo, result.tripCount, result.studyLogCount),
                ),
            )
    }
    ...
    @Operation(
        ...
        description = """
            ...
            - 요청 값의 originFilename은 꼭 파일 확장자를 포함한 파일명으로 요청해야 합니다.
        """,
    )
    @PostMapping("/profile-images/presigned")
    fun presigned(
        @AuthenticationPrincipal memberId: String,
        @RequestBody @Valid request: PresignProfileImageRequest,
    ): ResponseEntity<StandardResponse> {
        val result = memberFacade.issuePresignedUrl(memberId.toLong(), request)
    ...
    @Operation(
        ...
        description = """
            ...
            유효하지 않은 이미지 크기/확장자 등 도메인 정책을 위반해 실패할 경우 임시 경로에 저장된 이미지가 즉시 삭제되며 다시 업로드부터 수행해야 합니다.
    ...
    fun confirm(
        @AuthenticationPrincipal memberId: String,
        @RequestBody @Valid request: ConfirmProfileImageRequest,
    ): ResponseEntity<StandardResponse> {
        memberFacade.confirmImage(memberId.toLong(), request)
