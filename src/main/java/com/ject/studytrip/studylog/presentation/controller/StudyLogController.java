...
public class StudyLogController {
    ...
    public ResponseEntity<StandardResponse> presigned(
            @PathVariable @NotNull(message = "학습 로그 ID는 필수 요청 파라미터입니다.") Long studyLogId,
            @RequestBody @Valid PresignStudyLogImageRequest request) {
        PresignedStudyLogImageInfo info = studyLogFacade.issuePresignedUrl(studyLogId, request);
        return ResponseEntity.ok()
                .body(
                        StandardResponse.success(
                                HttpStatus.OK.value(),
                                PresignedStudyLogImageResponse.of(
                                        info.studyLogId(), info.tmpKey(), info.presignedUrl())));
    }
    ...
    public ResponseEntity<StandardResponse> confirm(
            @PathVariable @NotNull(message = "학습 로그 ID는 필수 요청 파라미터입니다.") Long studyLogId,
            @RequestBody @Valid ConfirmStudyLogImageRequest request) {
